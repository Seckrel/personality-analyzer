...
const TableBody = ({ data, handleGraphView, displayRange }) => (
    <tbody>
        {arrayRange(data, displayRange.start, displayRange.end).map((datum, index) => (
            <tr
                key={`${index}-${datum[0]}`} r
...
function ResumeAnalysis() {
    ...
    const displayRecordsIndexes = useMemo(() => ({
        ...
        "end": (SIZE_PER_PAGE * activePage) < state.data.length ? SIZE_PER_PAGE * activePage : state.data.length
    }), [activePage, state]);
